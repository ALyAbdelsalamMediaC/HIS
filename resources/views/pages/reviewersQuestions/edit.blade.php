@extends('layouts.app')
@section('title', 'HIS | Edit Reviewers Questions')
@section('content')
...
<section>
  ...
  <div class="gap-3 d-flex align-items-center">
    ...
    <div>
      <h2 class="h2-semibold" style="color:#35758C;">Edit Reviewers Questions</h2>
      <p class="h5-ragular" style="color:#ADADAD;">Edit and organize your survey questions.</p>
    </div>
  </div>
  ...
  <!-- Create New Question (same as add) -->
  <div class="create-question-containuer">
    ...
    <form method="POST" action="{{ route('reviewersQuestions.add') }}" novalidate class="mt-3">
      ...
      <input type="hidden" id="form_question_group_id" name="question_group_id" value="{{ $questionGroup->id }}">
  ...
  <x-modal id="editGroupNameModal" title="Edit Group Name">
    <form method="POST" action="{{ route('question_groups.update', $questionGroup) }}" class="mt-4" novalidate>
      @csrf
      @method('PUT')
      <div class="form-infield">
        <x-text_label for="group_name" :required="true">Group Name</x-text_label>
        <x-text_input type="text" id="group_name" name="name" placeholder="Group Name" value="{{ $questionGroup->name }}" data-required="true" data-name="Group Name" />
  ...
  <x-modal id="editQuestionModal{{ $question->id }}" title="Edit Question">
    <form method="POST" action="{{ route('reviewersQuestions.edit', $question->id) }}" class="mt-4" novalidate>
      @csrf
      @method('PUT')
      <input type="hidden" name="question_group_id" value="{{ $questionGroup->id }}">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.addEventListener('click', function(e) {
        ...
        // Only process clicks within edit modals only
        if (addBtn && addBtn.closest('[id^="editQuestionModal"]')) {
          ...
          // Show Add button in new row, hide in current
