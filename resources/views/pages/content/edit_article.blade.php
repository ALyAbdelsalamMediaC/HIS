@extends('layouts.app')
@section('title', 'HIS | Edit Article')
@section('content')
  <section>
    ...
    <div>
      <h2 class="h2-semibold" style="color:#35758C;">Edit Article</h2>
      <p class="h5-ragular" style="color:#ADADAD;">Update your article details</p>
    </div>
    </div>

    <form method="POST" action="{{ route('articles.update', $article->id) }}" enctype="multipart/form-data" novalidate
    class="mt-4">
    @csrf
    @method('PUT')
    ...
    <div class="form-infield">
      ...
      <div style="position: absolute; right: 0; top: 50%; transform: translateY(-50%); padding-right: 16px;">
        <x-button type="button" onclick="document.getElementById('iamge_path').click()">Choose file</x-button>
      </div>
      </div>
    ...
    <div class="form-infield">
      ...
      @if($article->pdf)
      <div class="mt-2">
      <a href="{{ asset($article->pdf) }}" target="_blank">View Current PDF</a>
      </div>
    @endif
    ...
    <div class="form-infield">
      ...
      value="{{ old('hyperlink', $article->hyperlink) }}" />
...
@push('scripts')
  ...
  <script>
    ...
    const fileInputs = [
      { id: 'iamge_path', placeholder: 'Choose an thumbnail from your gallery' },
