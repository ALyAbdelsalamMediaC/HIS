...
@section('content')
  ...
  <section>
    ...
    <div>
      <h2 class="h2-semibold" style="color:#35758C;">Add Help</h2>
      <p class="h5-ragular" style="color:#ADADAD;">Add new help</p>
    </div>
    </div>

    <form action="{{ route('policies.store') }}" method="POST" class="mt-4" novalidate>
    @csrf
    <div class="form-infield">
      <x-text_label for="title" :required="true">Help Title</x-text_label>
      <x-text_input type="text" id="title" name="title" placeholder="Help Title" value="{{ old('title') }}"
      data-required="true" data-name="Help Title" />
    ...
    return [$category->id => $category->title];
        ...
        data-name="Help Category" :selected="old('category_id')" />
    ...
    <div class="mt-3 form-infield">
      ...
      data-name="Help body">{{ old('body') }}</x-textarea>
    ...
    <div class="gap-3 mt-4 d-flex justify-content-end align-items-center">
      ...
      <x-button type="submit" class="px-4">Create Help</x-button>
    </div>
    </form>

    <!-- Add Category Modal -->
    <x-modal id="addCategoryModal" title="Add Category">
    <form action="{{ route('policies.categories.store') }}" method="POST" id="addCategoryForm">
      @csrf
      <div class="form-infield">
